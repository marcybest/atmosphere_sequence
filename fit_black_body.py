...
def black_body(wav, T, A):
    """
    wav : wavelength in meters
    T   : temperature in Kelvin
    A   : scaling factor (normalization)
    """
    return A * (2*h*c**2 / wav**5) / (np.exp(h*c/(wav*k*T)) - 1)
...
for plot_range in [[1.00, 1.35], [1.4, 1.8], [1.95, 2.5]]:
    ...
    plt.errorbar(wav[ind_plot], flx[ind_plot], yerr=err[ind_plot], c='0.7', zorder=1)
    plt.plot(wav[ind_plot], flx[ind_plot], c='k', zorder=2)
...
# Fit (initial guess: T = 1000 K, A = maximum flux)
popt, pcov = curve_fit(black_body,             # function used for the fit
                       wav*1e-6,               # data wavelength converted to m
                       flx,                    # data flux
                       p0=[1000, max(flx)],    # initial guess
                       sigma=err,              # data errors
                       absolute_sigma=True)

T_fit, A_fit = popt                     # estimated values
T_err, A_err = np.sqrt(np.diag(pcov))   # estimated errors
print(f"Fitted temperature: {T_fit:.2f} +/- {T_err:.2f} K")

# --------

# Plot of the model
model = black_body(wav*1e-6, T_fit, A_fit)
plt.plot(wav, model, c='r', zorder=3)
...
plt.plot([], [], c='r', zorder=3, label='Black body law')
plt.legend()
#plt.yscale('log')
#plt.xlim(1.2,1.3)
plt.show()
